...
const PlanForm = ({ setApplicationForm }: props) => {
  ...
  const Styles = {
    ...
    PlansContainer: 'mt-5 flex flex-col items-center lg:flex-row lg:items-stretch lg:justify-between text-gray-900 font-medium',
  ...
  return (
    <main className='h-4/5 max-w-screen-xl text-center bg-bgLight p-8 rounded-2xl'>
      <h2 className='mb-2'>Calculate price</h2>
        ...
        {LandingPageDetails.Pricing.plans.map((plan, index) => (
          <div className={`${Styles.Plan}`} key={index}>
            <div>
              ...
              <p className='mt-4'>
                Your price: $<span>{trackerNumber * parseInt(plan.price[0].substring(1))}</span>
                /month
              </p>
            </div>
      ...
      <p className='mt-4'>Plan selction and pricing will be finizled after contacting us.</p>
      <Button className='mt-2' size='lg' onClick={() => setApplicationForm((state) => state + 1)}>
        Next
      </Button>
    </main>
  )
